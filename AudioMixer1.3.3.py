...
# Label to display the current volume above the slider
volume_label = ttk.Label(frame, text="Volume: ", font=("Helvetica", 12))
volume_label.grid(row=1, column=0, columnspan=11)

# Volume slider
volume_slider = ttk.Scale(frame, from_=0, to=100, orient='horizontal', command=update_volume, style="Custom.Horizontal.TScale")
volume_slider.grid(row=2, column=0, columnspan=11, padx=5, pady=5, sticky=(tk.W, tk.E))

# Frame for scale labels
...
refresh_button.grid(row=5, column=0, columnspan=2, padx=5, pady=5, sticky=(tk.W, tk.E))
...
volume_up_key_label.grid(row=6, column=0, sticky=(tk.W))
...
volume_up_key_menu.grid(row=6, column=1, sticky=(tk.W, tk.E))

volume_down_key_label = ttk.Label(frame, text="Volume Down Key:", style="Custom.TLabel")
volume_down_key_label.grid(row=7, column=0, sticky=(tk.W))
...
volume_down_key_menu.grid(row=7, column=1, sticky=(tk.W, tk.E))
...
cycle_key_label.grid(row=8, column=0, sticky=(tk.W))
...
cycle_key_menu.grid(row=8, column=1, sticky=(tk.W, tk.E))
...
cycle_favorites_key_label.grid(row=9, column=0, sticky=(tk.W))
...
remove_favorite_button.grid(row=12, column=1, padx=5, pady=5, sticky=(tk.W, tk.E))
...
credits_label.grid(row=13, column=0, columnspan=2)
...
def change_session(event, use_favorites=False):
    if use_favorites:
        ...
        if favorites_listbox.get(next_index) == "Master Volume":
            ...
            for session in sessions:
                if session.Process.name() == favorites_listbox.get(next_index):
                    volume_label.config(text=f"Volume: {session.SimpleAudioVolume.GetMasterVolume() * 100:.0f}")
                    volume_slider.set(session.SimpleAudioVolume.GetMasterVolume() * 100)
                    break
        ...
        if next_index == 0:
            ...
            volume_label.config(text=f"Volume: {selected_session.SimpleAudioVolume.GetMasterVolume() * 100:.0f}")
            volume_slider.set(selected_session.SimpleAudioVolume.GetMasterVolume() * 100)
    ...
    if selected_index:
        ...
        if favorite_name == "Master Volume":
            ...
            if sign == '+':
                ...
                set_master_volume(max(0.0, current_volume - 0.05))
            # Update the slider value
            volume_label.config(text=f"Volume: {master_volume.GetMasterVolumeLevelScalar() * 100:.0f}")
            volume_slider.set(master_volume.GetMasterVolumeLevelScalar() * 100)
        else:
            for session in sessions:
                if session.Process.name() == favorite_name:
